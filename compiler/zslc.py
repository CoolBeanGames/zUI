...
def _native_events(prog: Program) -> list[str]:
    events: set[str] = set()
    def visit(n: Node):
        if n.event:
            events.add(n.event)
...
def _cs_node(n: Node, depth: int = 0) -> str:
    attrs = dict(n.attrs)
    if n.bind:
        attrs["bind"] = n.bind
    if n.source:
        attrs["source"] = n.source
    if n.event:
        attrs["on"] = n.event
    for flag in sorted(n.flags):
        attrs[flag] = "true"
    attr_text = ", ".join(f"[{_cs(k)}] = {_cs(v)}" for k, v in attrs.items())
    children = ", ".join(_cs_node(c, depth + 1) for c in n.children)
    return (f"new ZUI.ZuiNode({_cs(n.name)}, {_cs(n.text or '')}, "
            f"new System.Collections.Generic.Dictionary<string,string> {{ {attr_text} }}, "
            f"new ZUI.ZuiNode[] {{ {children} }})")


def gen_csharp(prog: Program, class_name: str, namespace: str, asset_base: str = "") -> str:
    roots = ",\n                ".join(_cs_node(n) for n in prog.roots)
    handlers = _native_events(prog)
    hooks = "\n".join(
        f'        partial void On_{h.replace(".", "_")}(string payload);' for h in handlers)
    wires = "\n".join(
        f'            host.On({_cs(h)}, p => On_{h.replace(".", "_")}(p));' for h in handlers)
...
def _cpp_node(n: Node) -> str:
    ...
    for flag in sorted(n.flags):
        attrs[flag] = "true"
    pairs = ", ".join("{" + _cpp(k) + ", " + _cpp(v) + "}" for k, v in attrs.items())
    kids = ", ".join(_cpp_node(c) for c in n.children)
    return f"zui::Node{{{_cpp(n.name)}, {_cpp(n.text or '')}, {{{pairs}}}, {{{kids}}}}}"


def gen_cpp(prog: Program, func: str, asset_base: str = "") -> str:
    roots = ",\n        ".join(_cpp_node(n) for n in prog.roots)
    handlers = _native_events(prog)
    wires = "\n".join(
...
def main(argv: list[str]) -> int:
    ...
    ap.add_argument("--backend", choices=["csharp", "cpp"], default="csharp")
